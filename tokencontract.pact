(namespace "free")
(define-keyset "free.testcontract-admin" (read-keyset "testcontract-admin"))
(module testtokencontract GOVERNANCE
  ...
  (defcap GOVERNANCE()
    ...
    (enforce-keyset "free.testcontract-admin")
  )
  ...
  (defun transfer-to-account(account:string amount-tokens:decimal)
    (with-capability (IS_ADMIN)
      (let ((g (at 'guard (coin.details account))))
      (install-capability (TRANSFER TKN_OWNER_ACC account amount-tokens))
      (transfer-create TKN_OWNER_ACC account g amount-tokens)
      )
    )
  )

  (defun transfer-to-airdrop-account(amount-tokens:decimal)
    (with-capability (IS_ADMIN)
      (install-capability (TRANSFER TKN_OWNER_ACC free.testtokenairdrop.AIRDROP_BANK amount-tokens))
      (transfer-create TKN_OWNER_ACC free.testtokenairdrop.AIRDROP_BANK (at 'guard (coin.details account)) amount-tokens)
    )
  )
...
; Successful Server result: "Loaded module free.testy, hash PWjgs86ZjTD9a1a0dTgmtAPpprYC_i58504EfGeWDyM"
